...
- **`dev`** — shared repository `ghcr.io/blackfuel-ai/bf-vllm/vllm-<arch>`, label-routed (`ai.bf-vllm.build.channel=dev`), never auto-deployed. Per-PR builds tag `:dev-pr-N-<sha>`; a manual dev build tags `:<base-version>-dev-g<sha7>` (e.g. `:v0.23.1rc0_bf.0.1.0-dev-gaebd933`), the tree's clean `git describe` base plus the commit, so the engine — which shows the tag verbatim — displays a version, not a bare sha. `bf-tools/version.py:dev_image_tag()` derives it. See [ADR-0004](./adr/0004-build-channels-versioning-labels.md) 2026-06-22 dev-tag amendment.
