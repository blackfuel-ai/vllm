...
from the tree's ``git describe`` self-name plus the commit sha:
    ...
    Dev image tag:    v0.23.1rc0_bf.0.1.0-dev-gaebd933
...
# ``v0.23.1rc0_bf.0.1.0-dev-gaebd933``). The base keeps the leading ``v`` and the
...
#
# A dev build pushes ``<base-version>-dev-g<sha7>`` to the shared
...
# the literal commit is carried as ``g<sha7>`` instead. Marker placement is
# ``-dev-`` BEFORE the sha so the version reads cleanly up to the marker.
# -----------------------------------------------------------------------------
...
def parse_dev_image_tag(tag: str) -> tuple[str, str]:
    ...
    Args:
        tag: e.g. ``v0.23.1rc0_bf.0.1.0-dev-gaebd933``.
    ...
    if not m:
        raise ValueError(
            ...
            "Expected shape: <base-version>-dev-g<sha7>, "
            "e.g. v0.23.1rc0_bf.0.1.0-dev-gaebd933"
        )
